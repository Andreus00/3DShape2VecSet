...
#SBATCH --gres=gpu:4
#SBATCH --output=./logs_ge_bce_bigger_5/myjob-%j.out       # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=./logs_ge_bce_bigger_5/myjob-%j.err        # File to which STDERR will be written - make sure this is not on $HOME
...
srun env -u SLURM_PROCID python3 -m torch.distributed.launch --nproc_per_node=4 \
	...
	--latent_vec_dim 64 --max_dist 0.1 --warmup_epoch 0 --lr 0.000001 \
	--save_every 1 --epochs 800 --output_dir output_bce_bigger_5 --log_dir output_bce_bigger_5 \
	--point_cloud_size 32768 --num_workers 8 \
	--resume output_bce_bigger/checkpoint-3.pth --start_epoch 4 \
