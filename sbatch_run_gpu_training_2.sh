...
#SBATCH --gres=gpu:8
#SBATCH --output=./logs_ge_2/myjob-%j.out       # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=./logs_ge_2/myjob-%j.err        # File to which STDERR will be written - make sure this is not on $HOME
...
srun env -u SLURM_PROCID python3 -m torch.distributed.launch --rdzv_endpoint=localhost:29400 main_ae_garmentcode.py --data_path ../GarmentCode/garmentcodedata_v2 --device cuda --batch_size 2 --accum_iter 4 --max_dist 0.1 --lr 0.0001 --save_every 1 --epochs 800 --output_dir output_v2 --log_dir output_v2 --start_epoch 4 --resume output_v2/checkpoint-3.pth
