...
class GridFiller:
    ...
    def fill_grid(
        self, udf_func: Callable[[Tensor], Tensor], max_batch: int, th_dist
    ) -> Tuple[Tensor, Tensor]:
        with torch.no_grad():
            ...
            for level, N in enumerate(self.N_levels):
                ...
                samples[mask_coarse_no_recompute, 3] = sample_udf(
                    udf_func, xyz, max_batch=max_batch, th_dist=th_dist
                )
...
def sample_udf(
    ...
    while start < coords.shape[0]:
        ...
        p = coords[start:end]
        if grad:
            udf[start:end], grads[start:end] = udf_func(p, th_dist)
        else:
            with torch.no_grad():
                udf[start:end], grads[start:end] = udf_func(p, -1)
        start = end

    # # Visualize gradients in 3D
    # import matplotlib.pyplot as plt

    # # Downsample for visualization if too many points
    # step = max(1, coords.shape[0] // 5000)
    # xyz_plot = coords[::step].cpu().numpy()
    # grads_plot = grads[::step].cpu().numpy()

    # # Normalize gradients for color mapping
    # norms = np.linalg.norm(grads_plot, axis=1, keepdims=True) + 1e-8
    # grads_norm = grads_plot / norms

    # # Map normalized gradients to RGB colors ([-1,1] -> [0,1])
    # colors = (grads_norm + 1) / 2

    # fig = plt.figure(figsize=(8, 6))
    # ax = fig.add_subplot(111, projection='3d')
    # ax.quiver(
    #     xyz_plot[:, 0], xyz_plot[:, 1], xyz_plot[:, 2],
    #     grads_plot[:, 0], grads_plot[:, 1], grads_plot[:, 2],
    #     length=0.05, normalize=True, colors=colors
    # )
    # ax.set_title("3D Gradients Visualization")
    # plt.show()
...
def get_mesh_from_udf(
    ...
    if not use_fast_grid_filler:
        ...
        udf, gradients = fast_grid_filler.fill_grid(udf_func, max_batch, th_dist=th_dist)
        # Visualize the grid: xyz as points, udf as color
        # import matplotlib.pyplot as plt

        # xyz = fast_grid_filler.samples[:, :3].cpu().detach().numpy()
        # udf_flat = udf.flatten().cpu().detach().numpy()

        # mask = udf_flat < 0.001

        # xyz = xyz[mask, :]
        # udf_flat = udf_flat[mask]

        # fig = plt.figure(figsize=(8, 6))
        # ax = fig.add_subplot(111, projection='3d')
        # p = ax.scatter(xyz[:, 0], xyz[:, 1], xyz[:, 2], c=udf_flat, cmap='viridis', s=1)
        # fig.colorbar(p, ax=ax, label='UDF Value')
        # ax.set_title('Grid Visualization (xyz, udf as color)')
        # plt.show()
    udf[udf < 0] = 0
    ...
    vertices, faces, _, _ = udf_mc_lewiner(udf, gradients, spacing=[spacing] * 3, th_alpha=th_alpha, th_beta=th_beta)
    ...
    udf, grads = sample_udf(udf_func, points, max_batch, grad=False, th_dist=th_dist)
    udf = udf.cpu().detach().numpy()
    ...
    final_verts = torch.tensor(mesh.vertices).float().to(device)
    final_faces = torch.tensor(mesh.faces).long().to(device)
    ...
    if differentiable:
        ...
        normals = torch.tensor(normals).float().to(device)
        ...
        edge_border = torch.tensor(edge_border).float().to(device)
        ...
        border_verts = torch.tensor(mesh.vertices[u_border]).float().to(device)
        xyz_s1_border = border_verts + 3 * th_dist * out_vec
        xyz_s2_border = border_verts - 3 * th_dist * out_vec
        s1_border, grads = sample_udf(udf_func, xyz_s1_border, max_batch, grad=False, th_dist=th_dist)
        s2_border, grads = sample_udf(udf_func, xyz_s2_border, max_batch, grad=False, th_dist=th_dist)
        s1_border = s1_border.unsqueeze(-1)
        s2_border = s2_border.unsqueeze(-1)
