...
#SBATCH -J ge-bce-no-bnd-sfc       # Job name
...
#SBATCH --gres=gpu:4
#SBATCH --output=./logs_ge_bce_no_bnd_no_sfc_no_grad/myjob-%j.out       # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=./logs_ge_bce_no_bnd_no_sfc_no_grad/myjob-%j.err        # File to which STDERR will be written - make sure this is not on $HOME
...
srun env -u SLURM_PROCID python3 -m torch.distributed.launch \
    ...
    --save_every 1 \
    --epochs 800 \
    --output_dir output_bce_no_bnd_no_sfc_no_grad \
    --log_dir output_bce_no_bnd_no_sfc_no_grad \
    --point_cloud_size 8192 \
    --num_samples 32768 \
    --latent_vec_size 2048 \
    --surface_samples_ratio 0.0 \
    --random_samples_ratio 0.5 \
    --surf_bnd_percent 0.0 \
    --surf_imp_percent 0.5 \
