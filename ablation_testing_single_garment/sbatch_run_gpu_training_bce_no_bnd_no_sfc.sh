...
#SBATCH --gres=gpu:4
#SBATCH --output=./logs_ge_bce_no_bnd_no_sfc/myjob-%j.out       # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=./logs_ge_bce_no_bnd_no_sfc/myjob-%j.err        # File to which STDERR will be written - make sure this is not on $HOME
...
srun env -u SLURM_PROCID python3 -m torch.distributed.launch \
    ...
    --save_every 1 \
    --epochs 800 \
    --output_dir output_bce_no_bnd_no_sfc \
    --log_dir output_bce_no_bnd_no_sfc \
