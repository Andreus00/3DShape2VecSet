...
#SBATCH --gres=gpu:4
#SBATCH --output=./logs_ge_bce_bigger_3/myjob-%j.out       # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=./logs_ge_bce_bigger_3/myjob-%j.err        # File to which STDERR will be written - make sure this is not on $HOME
...
srun env -u SLURM_PROCID python3 -m torch.distributed.launch \
	...
	--latent_vec_dim 64 --max_dist 0.1 --warmup_epoch 0 --lr 0.001 \
	--save_every 1 --epochs 800 --output_dir output_bce_bigger_3 --log_dir output_bce_bigger_3 \
